...
//must be same as other I2C device
...
void i2c_master_init(SSD1306_t * dev, int16_t sda, int16_t scl, int16_t reset)
{
    ...
    ESP_ERROR_CHECK(get_i2c_pins(I2C_NUM, &es_i2c_cfg));
    i2c_handle = i2c_bus_create(I2C_NUM, &es_i2c_cfg);
    assert(es_i2c_cfg.sda_io_num==sda);
    assert(es_i2c_cfg.scl_io_num==scl);
...
//	esp_err_t espRc = i2c_master_cmd_begin(I2C_NUM, cmd, 10/portTICK_PERIOD_MS);
	esp_err_t espRc = i2c_bus_cmd_begin(i2c_handle, cmd, 10/portTICK_PERIOD_MS);
	if (espRc == ESP_OK) {
...
//	i2c_master_cmd_begin(I2C_NUM, cmd, 10/portTICK_PERIOD_MS);
	i2c_bus_cmd_begin(i2c_handle, cmd, 10/portTICK_PERIOD_MS);
	i2c_cmd_link_delete(cmd);
...
//	espRc = i2c_master_cmd_begin(I2C_NUM, cmd, 10/portTICK_PERIOD_MS);
	espRc = i2c_bus_cmd_begin(i2c_handle, cmd, 10/portTICK_PERIOD_MS);
	if (espRc == ESP_OK) {
